...
class TomarOrdenPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final backgroundColor =
        isDark ? const Color(0xFF13131A) : Colors.grey[50];
        ...
        children: [
          ...
          Expanded(
            child: provider.visibleProducts.isEmpty
                ...
                : ListView.builder(
                    itemCount:
                        provider.visibleProducts.length,
                    itemBuilder: (context, index) {
                      final product =
                          provider.visibleProducts[index];
...
class _CartSection extends StatelessWidget {
  ...
  Future<void> _enviarOrden({
    ...
    final cartSnapshot =
        List<CartItem>.from(provider.cart);
    ...
    final cajaItems = cartSnapshot.map((item) {
      return CashItem(
    ...
    final nuevaOrden = RestaurantOrder(
      ...
      items: cocinaItems,
      totalAmount: total,
      notes: provider.notes.trim().isEmpty
          ? null
          : provider.notes.trim(),
    );
    ...
    bool agregadaAOrdenExistente = false;
    ...
    // Una mesa nueva pasa de libre a ocupada.
    if (orderType == OrderType.dineIn &&
        !provider.isExistingTable) {
      final mesasProvider =
    ...
    // Evita crear otra cuenta de caja para una orden existente.
    if (!agregadaAOrdenExistente) {
...
class _TypeButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final selected =
        context.select<TomarOrdenProvider, bool>(
