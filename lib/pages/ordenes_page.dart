                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          _buildTopStatusIndicator('Activas', '${provider.activeOrdersCount}', Colors.blue),
                          const SizedBox(width: 12),
                          _buildTopStatusIndicator('Listas', '${provider.readyOrdersCount}', Colors.green),
                        ],
                      )
                    ],
                  ),
                        ...
                        children: [
                          ...
                          Expanded(
                            ...
                            child: DropdownButtonFormField<String>(
                              value: provider.selectedFilterStatus,
                              ...
                              items: ['Todos', 'Pendiente', 'Preparando', 'Lista', 'Entregada', 'Cancelada']
                                  ...
                                  .toList(),
                              onChanged: (val) => provider.onStatusFilterChange(val ?? 'Todos'),
                            ),
                          ),
                          if (isDesktop) const VerticalDivider(width: 24, thickness: 1),
                          Expanded(
                  ...
                  Row(
                    children: [
                      Text('${provider.filteredOrders.length} orden(es) encontrada(s)', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.blueGrey)),
                    ],
                  ),
                  ...
                  if (provider.totalPages > 1)
                    Row(
                      ...
                      children: [
                        ...
                        Text('Página ${provider.currentPage} de ${provider.totalPages}', style: const TextStyle(fontWeight: FontWeight.bold)),
                        IconButton(
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(
                          mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            Text('Detalle de Comanda: ${provider.selectedOrderForModal!.id}', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                            IconButton(icon: const Icon(Icons.close), onPressed: provider.cerrarModal),
                          ],
                        ),
                        const Divider(),
                        const SizedBox(height: 8),
                        Text('Cliente/Mesa: ${provider.selectedOrderForModal!.tableOrCustomer}', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
                        Text('Tipo de Servicio: ${_getServiceLabel(provider.selectedOrderForModal!.serviceType)}', style: const TextStyle(color: Colors.blueGrey)),
                        Text('Hora de Registro: ${provider.selectedOrderForModal!.time} hrs'),
                        if (provider.selectedOrderForModal!.notes != null && provider.selectedOrderForModal!.notes!.isNotEmpty) ...[
                          const SizedBox(height: 8),
                          Container(
                            ...
                            child: Text('⚠️ Notas: ${provider.selectedOrderForModal!.notes}', style: const TextStyle(fontWeight: FontWeight.w500)),
                          )
                        ],
                        ...
                        Flexible(
                          child: ListView.builder(
                            shrinkWrap: true,
                            itemCount: provider.selectedOrderForModal!.items.length,
                            itemBuilder: (ctx, index) {
                              final item = provider.selectedOrderForModal!.items[index];
                              return Padding(
                        ...
                        Wrap(
                          ...
                          runSpacing: 8,
                          children: [
                            if (provider.selectedOrderForModal!.status == 'pendiente')
                              ElevatedButton.icon(
                            ...
                            if (provider.selectedOrderForModal!.status == 'preparando')
                              ElevatedButton.icon(
                            ...
                            if (provider.selectedOrderForModal!.status == 'lista')
                              ElevatedButton.icon(
                            ...
                            if (provider.selectedOrderForModal!.status != 'entregada' && provider.selectedOrderForModal!.status != 'cancelada')
                              OutlinedButton.icon(
