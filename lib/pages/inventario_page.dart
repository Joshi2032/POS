...
class _InventarioPageState extends State<InventarioPage> {
  ...
  void openEditor({Map<String, dynamic>? item}) {
            ...
            actions: [
              ...
              ElevatedButton(
                  onPressed: () {
                    ...
                    if (item == null) {
                      app.addInventoryItem(data);
                    } else {
                      app.updateInventoryItem(item['id'], data);
                    }
  ...
  Widget build(BuildContext context) {
    ...
    final items = app.inventory
        .where((i) =>
            search.isEmpty ||
            (i['name'] as String).toLowerCase().contains(search.toLowerCase()))
        .toList();
                  ...
                  : ListView.separated(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        return ListTile(
                          title: Text(it['name']),
                          subtitle:
                              Text('Stock: ${it['stock']} · ${it['category']}'),
                          trailing:
                              Row(mainAxisSize: MainAxisSize.min, children: [
                            IconButton(
                                icon: const Icon(Icons.edit),
                                onPressed: () => openEditor(item: it)),
                            IconButton(
                                icon: const Icon(Icons.delete),
                                onPressed: () => app
                                    .removeInventoryItem(it['id'] as String)),
                          ]),
                        );
                      },
                    ),
