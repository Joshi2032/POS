...
class PrinterService {
  ...
  static List<int> _armarDisenoTicket(Generator generator, RestaurantOrder orden) {
    ...
    // 2. Datos de la orden (Mesa / Mesero, Folio, Orden)
    // orden.tableOrCustomer trae el nombre real de la mesa resuelto vía join
    // con restaurant_tables. orden.waiterName trae el nombre del empleado
    // logueado en el momento de crear la orden (leído de profiles.full_name).
    const String personasFijo = '-'; // personas: pendiente de implementar

    final mesaTexto = 'MESA: ${_limpiarTexto(orden.tableOrCustomer)}';
    final meseroTexto = 'MESERO: ${_limpiarTexto(orden.waiterName ?? '-')}';
